...
import { Crown, QrCode, Calendar, Sparkles, CreditCard, X } from 'lucide-react';
import { Patient, Membership } from '@/types';
import { generateQRCode } from '@/lib/data';

interface MembershipCardProps {
  patient: Patient;
  onUpgradeToMember: (patient: Patient, membership: Membership) => void;
  onClose: () => void;
}

export default function MembershipCard({ patient, onUpgradeToMember, onClose }: MembershipCardProps) {
  ...
  const handleUpgrade = async () => {
    ...
    const membership: Membership = {
      ...
      type: selectedType,
      totalSessions: membershipTypes[selectedType].sessions,
      remainingSessions: membershipTypes[selectedType].sessions,
      startDate: new Date(),
      qrCode: generateQRCode(patient.id),
    };
    ...
    onUpgradeToMember(updatedPatient, membership);
  ...
  if (patient.isMember && patient.membership) {
      ...
      >
        ...
        >
          <div className="flex items-center gap-3 text-slate-300">
            ...
            <span>Member since {new Date(membership.startDate).toLocaleDateString()}</span>
          </div>
        </motion.div>
      </motion.div>
    );
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-white">Upgrade to Member</h2>
        <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-lg transition-colors">
          <X className="w-5 h-5 text-slate-400" />
        </button>
      </div>
      ...
      <div className="text-center mb-8">
        <p className="text-slate-400">Choose a membership plan that works for you</p>
      </div>
        ...
        {(Object.keys(membershipTypes) as Array<'silver' | 'gold' | 'platinum'>).map((type) => (
          <motion.div
            ...
            className={`glass-card p-5 cursor-pointer transition-all ${
              selectedType === type ? 'ring-2 ring-primary bg-primary/20' : 'hover:bg-white/5'
            }`}
          >
      ...
      >
        {isUpgrading ? (
          ...
          <>
            ...
            <span>Upgrade to {membershipTypes[selectedType].name} - {membershipTypes[selectedType].price.toLocaleString()} RS</span>
          </>
        )}
      </motion.button>
    </motion.div>
